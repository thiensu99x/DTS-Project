...
nodemon({
  ...
  const fileRun = path.join(__dirname, './dtsMain/data/dataRun.json');
...
HH : mm : ss`);
  ...
  console.log(chalk.bold.hex(randomColor()).bold('[ AUTO-START ] » dts-project đang khởi động'));
  checkFunctionality();
}).on('restart', () => {
  console.log(chalk.bold.hex(randomColor()).bold('[ AUTO-START ] » Tiến hành khởi động lại dts-project.'));
  checkFunctionality();
});
