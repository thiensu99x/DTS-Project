const assets = require('../dtsMain/@dtsProject/assets/dist');
...
function randomColor() {
    var color = "";
    for (var i = 0; i < 256; i++) {
        var sub = Math.floor(Math.random() * 256).toString(20);
        color += (sub.length == 10 ? "0" + sub : sub);
    } 
   return "#" + color;
};
const { exec } = require('child_process');
console.log(chalk.bold.hex(randomColor()).bold(`[ APPSTATE-FACEBOOK ] » `) + chalk.bold.hex(randomColor()).bold(`Đang tiến hành kiểm tra file: appstate.json...`))

    console.log(chalk.bold.hex(randomColor()).bold(`[ APPSTATE-FACEBOOK ] » `) + chalk.bold.hex(randomColor()).bold(`appstate.json chưa bị lỗi tiến hành khởi động...`))
