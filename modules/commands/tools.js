...
module.exports.handleReply = async function({
  ...
  switch (handleReply.type) {
    ...
    return api.sendMessage('⏳ Downloading, please wait...', event.threadID, (err, info) => {
      setTimeout(() => {
        api.sendMessage(`[😽] Ping: ${(Date.now() - timeStart)}ms \n[😽] Thời gian hoạt động: ${hours}:${minutes}:${seconds}`, event.threadID, event.messageID);
      }, 200);
    }, event.messageID);
}break;  
          case "13": {
...
var inbox = await api.getThreadList(100, null, ['INBOX']);
  ...
  for (var groupInfo of list) {
    ...
    return api.sendMessage(`[😽] Đã cập nhật dữ liệu của toàn bộ người dùng`, threadID)
}break;
        case "3": {
          const permission = ["100006272490820"];
             if (!permission.includes(event.senderID))
...
api.logout()
  ...
  return api.sendMessage(`[😽] Đã làm mới box và thành viên thành công!!!`, threadID, messageID)
}break;
          case "18": {
      const { threadID, messageID } = event;
...
return api.sendMessage(`[😽] ADMINBOT [😽]\n»============«\n\n${msg.join("\n")}\n\n————————🔱————————\n\n[😽] SUPPORTBOT [😽]\n»============«\n\n${msg1.join("\n\n")}`, event.threadID, event.messageID)
          ...
          case "5": {
            if (event.senderID != 100006272490820) return api.sendMessage(`[❗] Chúc bạn may mắn lần sau:))`, event.threadID, event.messageID)
      let number = [];
     ...
     //let number = [];
           ...
           return api.sendMessage(`[😽] Đã lọc những nhóm không tên hoặc dưới 20 thành viên.`,threadID)
  ...
  var threadInfo = await api.getThreadInfo(event.threadID);
	...
	var msg = `\n\n[😽] Bot đã Hoạt Động:${hours < 10 ? (hours > 0 ? " 0" + hours + " giờ" : 
...
return api.sendMessage("[😽] Đã hoàn tất reload config", event.threadID, event.messageID);    
}break;
          case "10 ":{
...
return api.sendMessage(`[😽] Chào bạn ${name}\n[😽] Chúc bạn một ngày tốt lành:\n[😽] Hôm này là: ${thu} || ${gio}\n[😽] Các múi giờ khác trên thế giới:\n-🇬🇧 London: ${gio2}\n-🇺🇸 New York: ${gio5}\n-🇰🇷 Seoul: ${gio3}\n-🇯🇵Tokyo: ${gio4}\n-🇧🇷 Brasília: ${gio1}\n-🇲🇾 Kuala Lumpur: ${gio6}\n-🇫🇷 Paris:${gio7}`, event.threadID, event.messageID);
    ...
    let xx = [...inbox].filter(group => group.isSubscribed && group.isGroup);
     ...
     return api.sendMessage({ body: '[😽] Top các box tương tác',attachment: createReadStream(path)},event.threadID,event.messageID);
}break;
          case "23": {
