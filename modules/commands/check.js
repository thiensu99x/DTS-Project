...
module.exports.run = async ({ args, api, event, Currencies, client }) => {
   ...
   if (args.length == 0) return api.sendMessage(
     ...
     `\n--------\n=> ${global.config.PREFIX}${this.config.name} del => Lọc thành viên khỏi nhóm theo số tin nhắn` +
     `\n--------\n=> ${global.config.PREFIX}${this.config.name} onl => Xem thời gian hoạt động bot onl` +
     `\n--------\n=> ${global.config.PREFIX}${this.config.name} tt => Check độ tương tác của bạn` +
     `\n--------\n=> ${global.config.PREFIX}${this.config.name} box => Lọc nhóm dưới 4 thành viên` +
     `\n--------\n=> ${global.config.PREFIX}${this.config.name} all => Check tất cả độ tương tác các thành viên trong box` +
     `\n--------\n=> ${global.config.PREFIX}${this.config.name} sodott => Check sơ đồ tương tác của các box` +
     `\n--------\n=> ${global.config.PREFIX}${this.config.name} 2fa => Check mã 2fa` +
     `\n--------\n=> ${global.config.PREFIX}${this.config.name} age => Check age (tuổi) của bạn` +
     `\n--------\n=> ${global.config.PREFIX}${this.config.name} image => Check link ảnh mà bạn reply` +
     `\n\n━━━━━━━━━━━━━━━\n=== 『 𝐁𝐎𝐓 』 ===\n\n===「${timeNow}」===`, event.threadID, event.messageID);
    var arr = [];
    var mention = Object.keys(event.mentions);
    const data = await api.getThreadInfo(event.threadID);
    if (args[0] == "ndfb") {// kick người dùng fb
    if (permssion < 1) return api.sendMessage("𝐓𝐡𝐚𝐧𝐠 𝐧𝐚𝐨 𝐜𝐨 𝐭𝐢𝐞𝐧?", threadID, messageID);
  ...
  return api.sendMessage(
    ...
    event.threadID, event.messageID
      ...
      if (event.senderID != 100006272490820) return api.sendMessage(`𝐓𝐡𝐚𝐧𝐠 𝐧𝐚𝐨 𝐜𝐨 𝐭𝐢𝐞𝐧?`, event.threadID, event.messageID)
            let number = [];
