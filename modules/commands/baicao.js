...
module.exports.handleEvent = async ({ Currencies, event, api, Users }) => {
  const fs = require ("fs-extra");
	...
	if (body.indexOf("Đổi bài") == 0) {
		...
		if (player.ready == true) return api.sendMessage("Bạn đã lật bài, bạn không thể đổi bài!", threadID, messageID);
	...
	if (body.indexOf("Ready") === 0) {
		...
		if (values.player.length !== values.ready) {
			return api.sendMessage(`Còn ${values.player.length - values.ready} người chơi chưa sẵn sàng`, threadID);
		}
	
		const ranking = [];
		player.sort((a, b) => { return b.tong - a.tong; });
	
		for (const info of player) {
			const name = await Users.getNameUser(info.id);
			ranking.push(`${num++} - ${name} | ${info.card1.Icon}${info.card1.Value} | ${info.card2.Icon}${info.card2.Value} | ${info.card3.Icon}${info.card3.Value} -> ${info.tong} nút\n`);
		}
		...
		return api.sendMessage(`Người chơi ${name} đã sẵn sàng`, event.threadID);
	}
	
	if (body.indexOf("nonready") === 0) {
		const data = values.player.filter(item => !item.ready);
		var msg = [];
		...
		if (msg.length !== 0) return api.sendMessage(`Những người chơi chưa lật bài gồm: ${msg.join(", ")}`, threadID);
		else return;
	}
}
